...
Name=$(curl -sS https://raw.githubusercontent.com/DanssBot/Generador-BOT/main/control | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/DanssBot/Generador-BOT/main/control | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
